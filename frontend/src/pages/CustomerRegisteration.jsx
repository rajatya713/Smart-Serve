...
const CustomerRegister = () => {
    ...
    return (
        <div className="flex overflow-hidden min-h-screen px-4 bg-linear-to-b from-white via-blue-50 to-blue-100 bg-[radial-gradient(#c1c1c1_1px,transparent_1px)] bg-size-[18px_18px] relative items-center justify-center sm:px-6 md:px-10">
            ...
            <div className="z-20 w-full max-w-md bg-white/70 backdrop-blur-xl border border-white/40 rounded-2xl shadow-2xl p-6 sm:p-8 md:max-w-lg md:p-10 fade-in">
                ...
                <form onSubmit={handleRegister} className="space-y-5 sm:space-y-6">
                    <div>
                        <label className="text-gray-700 text-sm sm:text-base font-medium">Full Name</label>
                        <input
                            type="text"
                            placeholder="Enter your full name"
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            className="w-full p-2.5 mt-1 text-sm sm:p-3 sm:text-base bg-white/80 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full p-2.5 mt-1 text-sm sm:p-3 sm:text-base bg-white/80 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none"
                            />
                            <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 text-lg">
                                {showPass ? "🙈" : "👁️"}
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        <label className="text-gray-700 text-sm sm:text-base font-medium">Confirm Password</label>
                        <input
                            type="password"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full py-2.5 sm:py-3 text-white font-semibold text-sm sm:text-base bg-blue-600 rounded-lg transition-all hover:bg-blue-700 duration-300 hover:scale-[1.03] disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100"
                    >
                ...
                <p className="mt-6 text-gray-600 text-center text-sm sm:text-base">
                    ...
                    <Link to="/customer/login" className="ml-1 text-blue-600 font-medium underline hover:text-blue-800">
                        Login
                    </Link>
                </p>
            </div>
