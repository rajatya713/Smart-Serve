...
const BookingPage = () => {
    ...
    const handleBooking = async (e) => {
        ...
        try {
            ...
            navigate("/payment", {
                state: { booking: data, vehicle, totalPrice: data.totalPrice },
            });
        } catch {
    ...
    return (
        <div className="min-h-screen bg-linear-to-b from-white via-blue-50 to-blue-100 bg-[radial-gradient(#c1c1c1_1px,transparent_1px)] bg-size-[18px_18px] px-4 py-10 sm:px-6 md:px-10">
            ...
            <div className="max-w-7xl mx-auto flex items-center justify-between mb-10">
                ...
                <button onClick={() => navigate(-1)} className="text-sm font-medium text-blue-600 hover:text-blue-800 underline cursor-pointer hover:scale-102">
                    ← Back
                </button>
            </div>
                    ...
                    <div className="md:col-span-3 bg-white/70 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl p-6 fade-in">
                        <form onSubmit={handleBooking} className="space-y-5">
                            ...
                            <div>
                                ...
                                <input
                                    type="date" min={today} value={pickupDate}
                            ...
                            <div>
                                ...
                                <input
                                    type="date" min={pickupDate || today} value={dropoffDate}
                            ...
                            <div>
                                ...
                                <input
                                    type="text" placeholder="e.g. MG Road, Lucknow" value={pickupLocation}
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                >
                                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${deliveryRequired ? "left-7" : "left-1"}`}></span>
                                </button>
                                <label className="text-sm text-gray-700 font-medium">Request delivery to my location</label>
                            </div>
                    ...
                    <div className="md:col-span-2 bg-white/70 backdrop-blur-xl border border-white/40 rounded-2xl shadow-xl p-6 h-fit fade-in">
                        ...
                        <div className="space-y-3 text-sm">
                            <div className="flex justify-between text-gray-600">
                                <span>Type</span><span className="font-medium text-gray-800">{vehicle.type}</span>
                            </div>
                            <div className="flex justify-between text-gray-600">
                                <span>Rate</span><span className="font-medium text-gray-800">₹{vehicle.pricePerDay}/day</span>
                            </div>
                            <div className="flex justify-between text-gray-600">
                                <span>Days</span><span className="font-medium text-gray-800">{days || "—"}</span>
                            </div>
                            <div className="flex justify-between text-gray-600">
                                <span>Delivery</span><span className="font-medium text-gray-800">{deliveryRequired ? "Yes" : "No"}</span>
                            </div>
