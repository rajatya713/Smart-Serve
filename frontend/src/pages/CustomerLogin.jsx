...
const CustomerLogin = () => {
  ...
  return (
    <div className="flex overflow-hidden min-h-screen px-4 bg-linear-to-b from-white via-blue-50 to-blue-100 bg-[radial-gradient(#c1c1c1_1px,transparent_1px)] bg-size-[18px_18px] relative items-center justify-center sm:px-6 md:px-10">
      ...
      {toast && <div className="z-50 px-6 py-3 text-white text-sm bg-green-600 rounded-lg shadow-lg fixed top-4 left-1/2 -translate-x-1/2 fade-in sm:text-base">{toast}</div>}
      {error && <div className="z-50 px-6 py-3 text-white text-sm bg-red-600 rounded-lg shadow-lg fixed top-4 left-1/2 -translate-x-1/2 fade-in sm:text-base">{error}</div>}
      <div className="z-20 w-full max-w-md p-6 bg-white/70 border border-white/40 rounded-2xl shadow-2xl backdrop-blur-xl fade-in sm:p-8 md:max-w-lg md:p-10">
        <Link to="/" className="flex mb-4 text-blue-700 font-medium text-sm hover:text-blue-900 items-center gap-2 sm:text-base">← Back to Home</Link>
        <div className="flex flex-col mb-6 text-center items-center sm:mb-8">
          <div className="mb-3 text-4xl sm:text-5xl">🔐</div>
          <h1 className="text-2xl font-extrabold text-transparent bg-linear-to-r from-blue-600 to-purple-600 bg-clip-text sm:text-3xl">Welcome Back!</h1>
          <p className="mt-2 text-gray-600 text-sm sm:text-base">Login to continue your SmartServe journey.</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-5 sm:space-y-6">
          ...
          <div className="flex justify-end">
            <Link to="/forgot-password" className="text-sm text-blue-600 hover:text-blue-800 underline sm:text-base">
              Forgot Password?
            </Link>
          </div>
        ...
        <p className="mt-6 text-gray-600 text-center text-sm sm:text-base">
          ...
          <Link to="/customer/register" className="ml-1 text-blue-600 font-medium underline hover:text-blue-800">Register</Link>
        </p>
      </div>
    </div>
  );
